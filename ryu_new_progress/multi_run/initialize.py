...
def make_weekly_momentum_mask(price_df:pd.DataFrame, mktcap_df:pd.DataFrame, vol_df:pd.DataFrame, n_group:int, day_of_week:str,
    ...
    mask = screener(mktcap_df,vol_df, mktcap_thresh, vol_thresh) # Daily 마스크를 받는다
    mktcap_screened = mktcap_df * mask
    start_idx = mktcap_screened.dropna(axis=1, thresh=1).index[0]
    #mktcap_screened_df = mktcap_screened[start_idx:] # 스크리닝을 통과한 mktcap
        
    # 주간 데이터를 생성합니다
    weekly_rtn = price_df.pct_change(7,fill_method=None).resample("W-" + day_of_week).last()[start_idx:]
    weekly_mask = mask.loc[weekly_rtn.index] #여기서 잘못된 거 맞는 것 같음 (인덱스 기반으로 수정해야할 것 같은데)
