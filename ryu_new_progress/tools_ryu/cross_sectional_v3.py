...
class CrossSectional:
    ...
    def __simulate_strategy(self, group_weight:pd.DataFrame):
        ...
        strategy_rtn = {}
        pf_value = 1
        dollar_value = np.zeros_like(group_weight.iloc[0])
        
        for t in group_weight.index:
            target_dollar_value = group_weight.loc[t] * pf_value    
            dollar_fee = ((target_dollar_value - dollar_value).abs() * self.fee_rate).sum()
            dollar_value = group_weight.loc[t] * (pf_value - dollar_fee) # 포트폴리오가 담을 각 코인의 달러가치
            
            ## t+1기 부터 t+7기 close까지 수익을 계산해야 한다 (t+7이 다음 리벨런싱날임)
    ...
    def make_market_index(self, mktcap_value=None, vol_value=None, fee_rate=0):
        '''
