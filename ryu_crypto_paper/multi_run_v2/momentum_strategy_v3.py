...
from multi_run_v2.backtest_v3 import simulate_longonly, simulate_longshort   # 벡테스팅 버전 바뀌면 여기를 수정해야함
...
def weekly_momentum_value_weighted_capped(price_df:pd.DataFrame, mktcap_df:pd.DataFrame, vol_df:pd.DataFrame,
    ...
    for key, mask in group_mask_dict.items():   
        ...
        if (freq == "weekly") or (freq == "Weekly"):
            ...
            final_value["Long_" + str(key)] = simulate_longonly(group_weight_df=(group_weight/7), daily_rtn_df=daily_rtn_df, fee_rate=fee_rate)
    ...
    if (freq == "weekly") or (freq == "Weekly"):
        ...
        final_value["LS-cross"] = simulate_longshort(long_weight_df=(group_weight_list[-1]/7), short_weight_df=(group_weight_list[0]/7),
                                                     daily_rtn_df=daily_rtn_df, fee_rate=fee_rate, margin="cross")
        final_value["LS-isolate"] = simulate_longshort(long_weight_df=(group_weight_list[-1]/7), short_weight_df=(group_weight_list[0]/7),
                                                     daily_rtn_df=daily_rtn_df, fee_rate=fee_rate, margin="isolate")
...
def weekly_momentum_volume_weighted_capped(price_df:pd.DataFrame, mktcap_df:pd.DataFrame, vol_df:pd.DataFrame,
                                           ...
                                           mktcap_thresh=None, vol_thresh=None, fee_rate=0, num_cap=5, freq="daily"):
    '''
    Volume Weighted로 투자 비중을 생성합니다 (상위 num_cap만큼 marketcap을 제한 해준다)
    ...
    daily_rtn_df, mktcap_used, group_mask_dict = inner_data_pp(price_df=price_df, mktcap_df=mktcap_df, vol_df=vol_df,
                                                               n_group=n_group, day_of_week=day_of_week, 
                                                               number_of_coin_group=number_of_coin_group, mktcap_thresh=mktcap_thresh,
                                                               vol_thresh=vol_thresh, freq=freq)  # initialize_v2에 있음
    
    ######## 처음에 Marketcap을 Cap 개수에 맞춰서 수정한다 ######
    idx = mktcap_used.index
    
    vol_df_used = vol_df.loc[idx].copy()
    min = vol_df_used.apply(lambda row: row.nlargest(num_cap).min(), axis=1) # 5번쨰로 큰 마켓켑을 구한다
    top_indices = vol_df_used.apply(lambda row: row.nlargest(num_cap).index.to_list(), axis=1) # 5번째까지 큰 애들의 위치를 가져온다
    
    # 값을 변경
    for idx, lst_of_top in top_indices.items():
        vol_df_used.loc[idx, lst_of_top] = min[idx]
    #########################################################
    
    for key, mask in group_mask_dict.items():   
        # 그룹의 marketcap 개산
        group_vol = vol_df_used * mask
        # Weight를 계산
