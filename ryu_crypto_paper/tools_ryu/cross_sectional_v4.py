...
class CrossSectional:
    ...
    def __simulate_strategy_long_short(self, long_weight_df:pd.DataFrame, short_weight_df:pd.DataFrame):
        ...
        for idx, row in self.daily_rtn.loc[self.strategy_start:].iterrows(): # Daily로 반복문을 돌린다
            # 수익률 평가가 리밸런싱보다 선행해야함
            pf_value_long, pf_value_short = (pf_value/2), (pf_value/2) # Portfolio의 절반만큼 Long-short
            
            # 롱전략 수익 평가
            dollar_value_long = long_weight * pf_value_long 
            dollar_value_long = dollar_value_long * (1+np.nan_to_num(row)) # update the dollar value
            long_weight = dollar_value_long / dollar_value_long.sum()   # update the weight 
            pf_value_long = dollar_value_long.sum() # update the pf value

            # 숏전략 수익 평가
            dollar_value_short = short_weight * pf_value_short 
            value = dollar_value_short * (1+np.nan_to_num(row*-1))
            dollar_value_short = np.where(value>0, value, 0)    # update the dollar value (청산 가치가 음수로 내려가는 것을 방지)
            short_weight = dollar_value_short / dollar_value_short.sum()   # update the weight 
            pf_value_short = dollar_value_short.sum() # update the pf value   
            
            pf_value = pf_value_long+pf_value_short 

            if idx in rebalancing_idx: # Rebalancing Date (장마감 직전에 리벨런싱 실시)
                long_weight = long_weight_df.loc[idx]   # target weight 
                short_weight = short_weight_df.loc[idx] # target weight
                
                target_dollar_value_long = np.nan_to_num(pf_value_long * long_weight)
                target_dollar_value_short = np.nan_to_num(pf_value_short * short_weight)
                
                dollar_fee_long = np.nansum(np.abs(target_dollar_value_long - np.nan_to_num(dollar_value_long)) * self.fee_rate)
                dollar_fee_short = np.nansum(np.abs(target_dollar_value_short - np.nan_to_num(dollar_value_short)) * self.fee_rate)
                
                pf_value_long = pf_value_long - dollar_fee_long # fee 차감
                pf_value_short = pf_value_short - dollar_fee_short
